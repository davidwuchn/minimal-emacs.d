...
(defvar my/gptel--fsm-metrics nil
  "Metrics tracking for FSM operations.

GOAL: Provide measurable outcomes for FSM usage patterns.
MEASURABLE OUTCOME: Track registrations, lookups, validations, and errors.

ASSUMPTION: Metrics enabled when non-nil.
ASSUMPTION: Hash table stores counters for each operation type.
BEHAVIOR: When enabled, increments counters on each operation.
EDGE CASE: Nil metrics disables tracking (no overhead).
TEST: (my/gptel--fsm-metrics-enable) => non-nil
TEST: After operations, counters reflect operation counts.

BUILDS ON DISCOVERY: Metrics enable data-driven optimization
of FSM operations based on actual usage patterns.

ADAPTS TO: Runtime enabling/disabling based on debugging needs.

Structure:
  :registrations - count of FSM registrations
  :lookups - count of FSM lookups by ID
  :validations - count of registry validations
  :errors - count of validation failures")

(defvar my/gptel--fsm-auto-validate nil
  "When non-nil, automatically validate registry after mutations.

GOAL: Catch registry corruption early in development/testing.
MEASURABLE OUTCOME: Zero undetected registry corruptions.

ASSUMPTION: Validation is expensive, disabled by default in production.
ASSUMPTION: Enabled during testing or debugging sessions.
BEHAVIOR: When non-nil, calls `my/gptel--fsm-registry-validate' after register/unregister.
EDGE CASE: Validation errors are signaled immediately (fail-fast).
TEST: (setq my/gptel--fsm-auto-validate t) enables auto-validation
TEST: After register, validation runs automatically.

BUILDS ON DISCOVERY: Early validation catches issues before they cascade
into wrong FSM selection in nested agent scenarios.

ADAPTS TO: Different environments (testing vs production) with
different validation requirements.

PROACTIVE MITIGATION: Fail-fast on corruption prevents subtle bugs
from propagating through nested agent calls.")
...
(defun my/gptel--fsm-metrics-enable ()
  "Enable FSM metrics tracking.

GOAL: Provide visibility into FSM operation patterns.
MEASURABLE OUTCOME: Metrics hash table created and populated.

ASSUMPTION: Metrics were previously disabled (nil).
BEHAVIOR: Creates metrics hash table with zeroed counters.
BEHAVIOR: Returns non-nil to indicate enabled state.
TEST: (my/gptel--fsm-metrics-enable) => hash-table
TEST: After enable, counters start at 0.

BUILDS ON DISCOVERY: Metrics enable data-driven optimization
of FSM operations based on actual usage patterns."
  (setq my/gptel--fsm-metrics
        (list :registrations 0
              :lookups 0
              :validations 0
              :errors 0)))

(defun my/gptel--fsm-metrics-disable ()
  "Disable FSM metrics tracking.

GOAL: Remove overhead when metrics not needed.
MEASURABLE OUTCOME: Metrics set to nil, no tracking overhead.

ASSUMPTION: Metrics were previously enabled.
BEHAVIOR: Sets metrics to nil.
BEHAVIOR: Returns nil to indicate disabled state.
TEST: (my/gptel--fsm-metrics-disable) => nil
TEST: After disable, no metrics tracking occurs.

ADAPTS TO: Production environments where overhead must be minimized."
  (setq my/gptel--fsm-metrics nil))

(defun my/gptel--fsm-metrics-inc (metric)
  "Increment a metrics counter.

ASSUMPTION: Metrics tracking is enabled (my/gptel--fsm-metrics non-nil).
ASSUMPTION: METRIC is a valid keyword (:registrations, :lookups, etc.).
BEHAVIOR: Increments the specified counter by 1.
BEHAVIOR: Silently does nothing if metrics disabled.
EDGE CASE: Metrics disabled - no operation, no error.
EDGE CASE: Unknown metric - no operation, no error.
TEST: (my/gptel--fsm-metrics-inc :registrations) increments counter
TEST: When metrics nil, function returns without error.

PROACTIVE MITIGATION: Silent failure when metrics disabled prevents
errors in production code that doesn't check metrics state."
  (when my/gptel--fsm-metrics
    (let ((idx (plist-member my/gptel--fsm-metrics metric)))
      (when idx
        (setcar (cdr idx) (1+ (cadr idx)))))))
    ...
    (let ((existing-id (gethash fsm my/gptel--fsm-registry)))
      (or existing-id
          (let ((id (my/gptel--fsm-generate-id)))
            ...
            (my/gptel--fsm-metrics-inc :registrations)
            (when my/gptel--fsm-auto-validate
              (condition-case err
                  (my/gptel--fsm-registry-validate)
                (error
                 (my/gptel--fsm-metrics-inc :errors)
...
in performance-critical nested agent scenarios."
  (my/gptel--fsm-metrics-inc :lookups)
...
Returns t on success, signals error on failure."
  (my/gptel--fsm-metrics-inc :validations)
