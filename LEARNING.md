...
- **The `Invalid function` Macro Trap**: If a macro (like `gptel-with-preset`) is not loaded into the environment *before* an Elisp file is byte-compiled, Emacs assumes it is a regular function and compiles it as such. At runtime, when the macro is actually loaded, executing the compiled code throws `Invalid function: gptel-with-preset` because it expects a compiled function but finds a macro. The fix is ensuring files containing macros are explicitly required at compile-time (e.g., `(require 'gptel)` at the top level).