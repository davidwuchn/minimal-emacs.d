...
- **DashScope Requires HTTP/1.1**: DashScope's `coding.dashscope.aliyuncs.com` endpoint fails with `Could not parse HTTP response` when curl uses HTTP/2. Adding `:curl-args '("--http1.1")` to the backend definition forces HTTP/1.1 and resolves the parse errors. This matches Moonshot's requirement for HTTP/1.1. When a backend returns parse errors, check if other backends with similar `:curl-args` work — the protocol version may be the culprit, not the API key or endpoint.
- **Empty String Tool Names**: Models can emit malformed tool calls with `name=""` (empty string), not just `nil` or `"null"`. The nil-tool detection predicate must check for all three: `(or (null name) (eq name :null) (equal name "null") (equal name ""))`. Both the FSM sanitization path (`gptel-ext-tool-sanitize.el`) and the message injection path (`gptel-ext-reasoning.el`) need this check to prevent 400 errors from downstream APIs.
...
- **Single Backend Simplicity**: Running one backend (DashScope/Qwen) for all modes eliminates an entire class of problems — multi-provider auth management, per-provider error format differences, routing logic bugs, and doubled debugging surface area. Only split to multiple providers when a single model demonstrably fails at a specific task category.
