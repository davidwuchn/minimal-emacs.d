...
- **Root-level `.elc` files break the init chain**: Stale compiled files at repo root (`early-init.elc`, `init.elc`) are especially insidious — they can cause `minimal-emacs--check-success` to fail with "Configuration error" because the init chain silently aborts mid-way. The batch test (`emacs --batch -l early-init.el -l init.el`) works, but interactive startup fails because `load-prefer-newer` doesn't catch all cases. Solution: `rm -f *.elc lisp/*.elc` to purge all compiled init files.
