...
workflow_action_elisp() {
    ...
    printf '(condition-case _err (let ((root (file-name-as-directory "%s")) (load-prefer-newer t)) (let ((inhibit-message t) (load-verbose nil)) (ignore-errors (load-file (expand-file-name "lisp/modules/gptel-tools-agent.el" root)))     (dolist (module (list "gptel-tools-agent-prompt-build.el" "gptel-tools-agent-error.el" "gptel-benchmark-subagent.el" "gptel-tools-agent-main.el" "gptel-auto-workflow-projects.el" "gptel-auto-workflow-evolution.el" "gptel-auto-experiment-ai-behaviors.el" "gptel-auto-workflow-ontology-router.el" "gptel-tools-agent-base.el")) (ignore-errors (load-file (expand-file-name (concat "lisp/modules/" module) root)))) (ignore-errors (when (fboundp (quote gptel-auto-workflow--activate-live-root)) (gptel-auto-workflow--activate-live-root root))) (ignore-errors (when (fboundp (quote gptel-auto-workflow--reload-live-support)) (gptel-auto-workflow--reload-live-support root)))) (condition-case action-err %s (error (format "[workflow-action] action-error: %%s" (error-message-string action-err))))) (error (format "[workflow-action] load-error: %%s" (error-message-string _err))))' \
           "$ROOT_LISP" "$dispatch"
}
