...
workflow_action_elisp() {
    ...
    printf '(condition-case _err (let ((root (file-name-as-directory "%s")) (load-prefer-newer t)) (let ((inhibit-message t) (load-verbose nil)) (ignore-errors (load-file (expand-file-name "lisp/modules/gptel-tools-agent.el" root)))     (dolist (module (list "gptel-tools-agent-prompt-build.el" "gptel-tools-agent-error.el" "gptel-benchmark-subagent.el" "gptel-tools-agent-main.el" "gptel-auto-workflow-projects.el" "gptel-auto-workflow-ontology-router.el")) (ignore-errors (load-file (expand-file-name (concat "lisp/modules/" module) root)))) (ignore-errors (when (fboundp (quote gptel-auto-workflow--activate-live-root)) (gptel-auto-workflow--activate-live-root root))) (ignore-errors (when (fboundp (quote gptel-auto-workflow--reload-live-support)) (gptel-auto-workflow--reload-live-support root)))) %s) (error (format "[workflow-action] action-error: %%s" (error-message-string _err))))' \
           "$ROOT_LISP" "$dispatch"
}
