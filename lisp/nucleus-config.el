...
(defconst nucleus--gptel-system-reminder-plan
  (string-join
   (list
    ...
    "□ Bash.cmd ∉ {sed, tee, echo, cat, >, >>, rm, mv, touch, git_commit}"
    "□ Edits = ∅ (ZERO exceptions)"
    ""
    "Responsibility ≜ λ(req)."
    "  action: think → read → search → delegate(explore) → plan"
    "  output: comprehensive ∧ concise ∧ ¬verbose"
    "  ambiguity ⟹ ask_user(clarification) ∨ ask_user(tradeoffs) ∧ ¬assume_intent"
    ""
    "□ ¬execute ∧ ¬modify_files ∧ ¬modify_system"
    "</system-reminder>")
   "\n")
