...
;; Keep the cron wrapper's `emacsclient --eval` payload short and let the
;; normal init path provide the core Gptel stack.  The wrapper only needs to
;; seed repo-local module paths, reload the workflow modules from the requested
;; worktree, and queue the chosen action.
