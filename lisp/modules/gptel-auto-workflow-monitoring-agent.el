...
(defvar gptel-auto-workflow--running nil) ;; from production.el — pipeline active flag
...
If any probe shows a problem, writes a mementum memory and returns :healthy
nil."
    ...
    (when (> n 0)
      ...
      (when (and (> total-deployed 0)
        ...
        (push (list :parameter 'gptel-auto-workflow-monitoring-impact-improvement-threshold
                    ...
                    :reason (format "Low keep-rate (%.1f%%) in last %d cycles (%d/%d deployments kept). Raise
impact-improvement-threshold to be more selective."
