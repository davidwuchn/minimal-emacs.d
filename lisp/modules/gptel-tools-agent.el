...
;;; Simple Auto-Evolve (Direct Branch Push)
;;; ====================

(defcustom gptel-auto-evolve-branch-prefix "auto-evolve"
  "Prefix for auto-evolve branches. Full name: {prefix}/{YYYY-MM-DD}."
  :type 'string
  :group 'gptel-tools-agent)

(defcustom gptel-auto-evolve-target-branch "main"
  "Target branch to merge into after review."
  :type 'string
  :group 'gptel-tools-agent)

(defcustom gptel-auto-evolve-verify-script "scripts/verify-nucleus.sh"
  "Script to run for verification (relative to project root)."
  :type 'file
  :group 'gptel-tools-agent)

(defcustom gptel-auto-evolve-max-attempts 3
  "Maximum improvement attempts per run."
  :type 'integer
  :group 'gptel-tools-agent)

(defcustom gptel-auto-evolve-timeout 600
  "Timeout for each improvement attempt (seconds)."
  :type 'integer
  :group 'gptel-tools-agent)

(defvar gptel-auto-evolve--run-id nil
  "Current run ID (YYYY-MM-DD format).")

(defvar gptel-auto-evolve--branch nil
  "Current auto-evolve branch name.")

(defvar gptel-auto-evolve--attempts 0
  "Number of improvement attempts in current run.")

(defvar gptel-auto-evolve--improvements nil
  "List of successful improvements in current run.")

(defun gptel-auto-evolve-run ()
  "Run simple auto-evolve cycle.

Flow:
1. Create branch: auto-evolve/{YYYY-MM-DD}
2. For each target in gptel-auto-workflow-targets:
   a. Run improvement agent
   b. Verify with tests
   c. If passed: commit, continue
   d. If failed: revert, try next target
3. Push branch to origin
4. Log results

Cron: emacsclient -e '(gptel-auto-evolve-run)'
Manual: M-x gptel-auto-evolve-run"
  (interactive)
  (message "\n[auto-evolve] ═════════════════════════════════════════════")
  (message "[auto-evolve] Starting auto-evolve run...")
  
  (let* ((default-directory (expand-file-name user-emacs-directory))
         (gptel-auto-evolve--run-id (format-time-string "%Y-%m-%d"))
         (gptel-auto-evolve--branch (format "%s/%s" 
                                             gptel-auto-evolve-branch-prefix
                                             gptel-auto-evolve--run-id))
         (gptel-auto-evolve--attempts 0)
         (gptel-auto-evolve--improvements nil)
         (targets gptel-auto-workflow-targets)
         (success-count 0)
         (fail-count 0))
    
    (unless (file-exists-p ".git")
      (error "[auto-evolve] Not in a git repository: %s" default-directory))
    
    (message "[auto-evolve] Run ID: %s" gptel-auto-evolve--run-id)
    (message "[auto-evolve] Branch: %s" gptel-auto-evolve--branch)
    (message "[auto-evolve] Targets: %s" (string-join targets ", "))
    
    (when (gptel-auto-evolve--create-branch)
      (dolist (target targets)
        (when (< gptel-auto-evolve--attempts gptel-auto-evolve-max-attempts)
          (message "\n[auto-evolve] ─────────────────────────────────────")
          (message "[auto-evolve] Target: %s" target)
          (condition-case err
              (if (gptel-auto-evolve--improve-target target)
                  (progn
                    (cl-incf success-count)
                    (push target gptel-auto-evolve--improvements))
                (cl-incf fail-count))
            (error
             (message "[auto-evolve] Error: %s" err)
             (cl-incf fail-count)))))
      
      (gptel-auto-evolve--finalize success-count fail-count))))

(defun gptel-auto-evolve--create-branch ()
  "Create auto-evolve branch from current main.
Returns t if successful, nil otherwise."
  (condition-case err
      (let ((default-directory (expand-file-name user-emacs-directory)))
        (message "[auto-evolve] Fetching origin...")
        (shell-command "git fetch origin --quiet")
        
        (message "[auto-evolve] Checking for existing branch...")
        (let ((existing (shell-command-to-string 
                         (format "git branch --list %s" gptel-auto-evolve--branch))))
          (when (string-match-p gptel-auto-evolve--branch existing)
            (message "[auto-evolve] Branch exists, deleting...")
            (shell-command (format "git branch -D %s 2>/dev/null || true" 
                                   gptel-auto-evolve--branch))))
        
        (message "[auto-evolve] Creating branch from origin/main...")
        (if (zerop (shell-command 
                    (format "git checkout -b %s origin/main" gptel-auto-evolve--branch)))
            (progn
              (message "[auto-evolve] ✓ Branch created: %s" gptel-auto-evolve--branch)
              t)
          (progn
            (message "[auto-evolve] ✗ Failed to create branch")
            nil)))
    (error
     (message "[auto-evolve] ✗ Branch creation error: %s" err)
     nil)))

(defun gptel-auto-evolve--improve-target (target)
  "Run improvement agent on TARGET file.
Returns t if improvement succeeded and verified, nil otherwise."
  (cl-incf gptel-auto-evolve--attempts)
  
  (let* ((target-path (expand-file-name target (expand-file-name user-emacs-directory)))
         (backup-file (concat target-path ".auto-evolve-backup"))
         (result nil))
    
    (unless (file-exists-p target-path)
      (message "[auto-evolve] ✗ Target not found: %s" target)
      (cl-return-from gptel-auto-evolve--improve-target nil))
    
    (message "[auto-evolve] Backing up: %s" target)
    (copy-file target-path backup-file t)
    
    (message "[auto-evolve] Running improvement agent...")
    (condition-case err
        (setq result (gptel-auto-evolve--run-agent target))
      (error
       (message "[auto-evolve] Agent error: %s" err)
       (rename-file backup-file target-path t)
       (cl-return-from gptel-auto-evolve--improve-target nil)))
    
    (if (and result (plist-get result :success))
        (progn
          (message "[auto-evolve] Agent completed, verifying...")
          (if (gptel-auto-evolve--verify)
              (progn
                (message "[auto-evolve] ✓ Verification passed")
                (delete-file backup-file)
                (gptel-auto-evolve--commit target (plist-get result :summary))
                t)
            (progn
              (message "[auto-evolve] ✗ Verification failed, reverting")
              (rename-file backup-file target-path t)
              nil)))
      (progn
        (message "[auto-evolve] ✗ Agent returned failure")
        (rename-file backup-file target-path t)
        nil))))

(defun gptel-auto-evolve--run-agent (target)
  "Run improvement agent on TARGET.
Returns plist with :success and :summary."
  (let* ((target-name (file-name-nondirectory target))
         (prompt (format "You are an auto-evolve agent optimizing Emacs Lisp code.
...
If no improvement needed, output:
NO-OP: <reason>" target-name))
         (result nil)
         (output nil))
    
    (condition-case err
        (progn
          (setq output (gptel-auto-evolve--call-agent prompt))
          (message "[auto-evolve] Agent output: %s" 
                   (truncate-string-to-width output 200 nil nil "..."))
          
          (cond
           ((string-match "IMPROVEMENT: \\([^\n]+\\)" output)
            (setq result (list :success t
                               :summary (match-string 1 output))))
           ((string-match "NO-OP:" output)
            (message "[auto-evolve] No improvement needed")
            (setq result (list :success nil :summary "No improvement needed")))
           (t
            (message "[auto-evolve] Unexpected output format")
            (setq result (list :success nil :summary "Unexpected output")))))
      (error
       (setq result (list :success nil :summary (format "Error: %s" err)))))
    
    result))

(defun gptel-auto-evolve--call-agent (prompt)
  "Call the agent with PROMPT using gptel.
Returns the agent's response text."
  ...
  (let ((response nil)
    ...
    (condition-case err
        (progn
          (gptel-request prompt
            :callback (lambda (response-text info)
                        (setq response response-text
                              callback-called t
                              done t)))
          (with-timeout (gptel-auto-evolve-timeout
                         (setq done t)
                         (error "Agent timeout"))
            (while (not done)
              (sit-for 0.1)))
          (or response ""))
      (error
       (error "Agent call failed: %s" err)))))

(defun gptel-auto-evolve--verify ()
  "Run verification script.
Returns t if verification passed, nil otherwise."
  (let* ((default-directory (expand-file-name user-emacs-directory))
         (script (expand-file-name gptel-auto-evolve-verify-script default-directory)))
    (unless (file-exists-p script)
      (message "[auto-evolve] ⚠ Verify script not found: %s" script)
      (cl-return-from gptel-auto-evolve--verify t))
    
    (message "[auto-evolve] Running: %s" script)
    (zerop (call-process "bash" nil nil nil script))))

(defun gptel-auto-evolve--commit (target summary)
  "Commit improvement to TARGET with SUMMARY."
  (let* ((default-directory (expand-file-name user-emacs-directory))
         (msg (format "◈ auto-evolve: %s
...
Summary: %s"
    ...
    (message "[auto-evolve] ✓ Committed: %s" summary)))

(defun gptel-auto-evolve--finalize (success-count fail-count)
  "Finalize the run with SUCCESS-COUNT and FAIL-COUNT."
  (let* ((default-directory (expand-file-name user-emacs-directory))
         (log-file (expand-file-name "var/tmp/cron/auto-evolve.log"
                                     default-directory)))
    
    (message "\n[auto-evolve] ═════════════════════════════════════════════")
    (message "[auto-evolve] Run Complete: %s" gptel-auto-evolve--run-id)
    (message "[auto-evolve] Successes: %d | Failures: %d" success-count fail-count)
    (message "[auto-evolve] Improvements: %s" 
             (string-join (or gptel-auto-evolve--improvements '("none")) ", "))
    
    (when (> success-count 0)
      (message "[auto-evolve] Pushing branch: %s" gptel-auto-evolve--branch)
      (if (zerop (shell-command 
                  (format "git push -u origin %s --force" gptel-auto-evolve--branch)))
          (message "[auto-evolve] ✓ Pushed to origin/%s" gptel-auto-evolve--branch)
        (message "[auto-evolve] ✗ Push failed")))
    
    (make-directory (file-name-directory log-file) t)
    (with-temp-file log-file
      (insert (format "[%s] Run: %s, Success: %d, Fail: %d, Improvements: %s\n"
