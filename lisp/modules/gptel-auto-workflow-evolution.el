  ...
  (when (and gptel-auto-workflow--self-healing-log
    ...
    (let* ((root (gptel-auto-workflow--worktree-base-root))
      ...
      (with-temp-file file
        ...
        (insert "Format: timestamp | diagnosis | remedy | before | after | effective?\n\n")
...
(defun gptel-auto-workflow--load-self-healing-state ()
  "Read persisted self-healing state.
Called on startup to restore learned thresholds."
  (when (fboundp 'gptel-auto-workflow--worktree-base-root)
    (let* ((root (gptel-auto-workflow--worktree-base-root))
           (file (expand-file-name gptel-auto-workflow--self-healing-state-file root)))
      (when (file-exists-p file)
        (message "[self-heal] Restoring state from %s"
...
(defun gptel-auto-workflow--run-diagnostic-probe ()
  "Run a trivial experiment to verify grader is working.
If grader fails on trivial change, the grader itself is broken.
Returns t if grader healthy, nil if broken."
  (message "[self-heal] Running diagnostic probe...")
  (let* ((probe-file (expand-file-name gptel-auto-workflow--probe-target
                                       (gptel-auto-workflow--worktree-base-root)))
         (fixture-content ";;; probe-fixture.el --- test fixture -*- lexical-binding: t; -*-\n(defun probe-test () 1)\n"))
    ;; Ensure fixture exists
    (make-directory (file-name-directory probe-file) t)
    (with-temp-file probe-file
      (insert fixture-content))
    ;; Create a trivial worktree
    (let* ((worktree (make-temp-file "probe-worktree" t))
           (target-file (expand-file-name "probe-fixture.el" worktree))
           (probe-healthy t))
      (make-directory worktree t)
      (with-temp-file target-file
        (insert fixture-content))
      ;; Trivial modification: add docstring
      (with-temp-file target-file
