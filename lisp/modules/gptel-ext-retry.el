...
FORCE-TRIM-P, when non-nil, bypasses the
`my/gptel-retry-keep-recent-tool-results'
nil check. Used by compaction passes to ensure trimming occurs even if the
user
          ...
          (when (my/gptel--bytes-threshold-met-p bytes-saved)
            (dolist (candidate to-truncate)
              (pcase-let ((`(,content-index ,part-index) candidate))
                (let* ((content-entry (elt contents content-index))
                       (parts (and (proper-list-p content-entry)
                                   (plist-get content-entry :parts)))
                        (part (and parts
                                   (sequencep parts)
                                   (not (stringp parts))
                                   (elt parts part-index)))
                       (function-response (and (proper-list-p part)
                                               (plist-get part :functionResponse)))
                       (response (and (proper-list-p function-response)
                                      (plist-get function-response :response))))
                  (when (proper-list-p response)
                    (plist-put response :content replacement)
                    (cl-incf truncated)))))))
      truncated))))

(defun my/gptel--trim-reasoning-content (info)
  "Strip reasoning_content from older assistant messages in INFO to reduce
payload.
...
When the active model/backend requires a reasoning field (for example
Moonshot's
`reasoning_content`), every assistant message with `:tool_calls` must carry
that
field, even if it is the empty string.

INFO is the request info plist containing :model, :backend, :data, and
:buffer.
...
PREDICATE is a function that takes a message plist and returns non-nil if it
matches.
Returns a list of indices in ascending order, or nil if PREDICATE is not a
function.
ASSUMPTION: PREDICATE is a valid function; nil or non-function returns nil
immediately.
...
EDGE CASE: Content that becomes empty after filtering is preserved as empty
vector.
...
Matches network failures, curl errors, gateway errors, model-side bugs, and
API cold starts.")
...
These indicate malformed tool arguments generated by the model, not user
errors.")
...
ASSUMPTION: Error messages can be in :message (plist) or 'message (alist)
keys."
  (cond
...
ASSUMPTION: HTTP status codes 408, 429, 500-504 are transient and safe to
retry.
ASSUMPTION: Error messages containing 'Malformed JSON', \='timeout\=',
\='Overloaded\=',
  'Too Many Requests' indicate temporary failures, not permanent errors.
ASSUMPTION: HTTP 400 with \='InvalidParameter\=' or 'function.arguments'
errors
...
EDGE CASE: Pattern variables may be nil during load order issues; guards
prevent errors.
...
EDGE CASE: my/gptel-max-retries=nil means retry indefinitely (capped at 30s
delay).
EDGE CASE: Partial insertion cleanup requires both :position and
:tracking-marker
...
TRIMMED-TOTAL-VAR is a symbol bound to total items trimmed (modified
in-place).
...
PASS-MSG is a format string for logging when items were trimmed (gets `n' and
`new-bytes' as args).
...
Uses `json-serialize' for accuracy. Returns 0 if :data is nil or serialization
fails."
  (let ((data (my/gptel--info-data info)))
    (if data
        (condition-case err
...
ASSUMPTION: Model names may include backend prefixes (e.g.,
\"moonshot/kimi-k2.6\")
