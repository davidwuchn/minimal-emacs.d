              ...
              (dolist (idx to-truncate)
                (let* ((msg (aref messages idx))
                  ...
                  (when (and (stringp content)
                    ...
                    (cl-incf bytes-to-save (- (string-bytes content) (string-bytes replacement)))))))
              (when (or (= my/gptel-trim-min-bytes 0)
                        (>= bytes-to-save my/gptel-trim-min-bytes))
                (dolist (idx (nreverse to-truncate))
                  (let* ((msg (aref messages idx))
                         (content (plist-get msg :content)))
                    (when (and (stringp content)
                               (> (string-bytes content) (string-bytes replacement)))
                  ...
                  (let* ((part (elt parts part-index))
                    ...
                    (when (and (stringp response-content)
                      ...
                      (push (list content-index part-index)
                            ordered-candidates)))))))
          (when ordered-candidates
            (let* ((ordered-candidates (nreverse ordered-candidates))
                   (to-truncate (seq-take ordered-candidates
                                          (max 0 (- (length ordered-candidates) keep))))
                   (bytes-to-save
                    (cl-loop for (cidx pidx) in to-truncate
                             sum (let* ((content-entry (elt contents cidx))
                                       (part (elt (plist-get content-entry :parts) pidx))
                                       (response-content
                                        (plist-get (plist-get part :functionResponse) :response)))
                                   (and (stringp response-content)
                                        (> (string-bytes response-content)
                                           (string-bytes replacement)))
                                   (- (string-bytes response-content)
                                      (string-bytes replacement))))))
