...
EDGE CASE: Non-numeric retries default to 0.
EDGE CASE: Nil/unbound constants fall back to defaults (4.0, 2.0, 30.0).
    ...
    (let* ((data (plist-get info :data))
      ...
      (when (and messages (> (length messages) keep))
        (let ((cutoff (- (length messages) keep))
              (truncation-text-len (length truncation-text)))
          (dotimes (i cutoff)
            (let* ((msg (aref messages i))
