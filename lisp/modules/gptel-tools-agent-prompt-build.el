...
(defvar gptel-auto-experiment--current-task-hint nil)
...
(declare-function gptel-auto-workflow--graph-suggest-questions "gptel-auto-workflow-memory-schema")
(declare-function gptel-auto-workflow--graph-community-for-target "gptel-auto-workflow-memory-schema")
...
(defvar gptel-auto-workflow--ab-test-sections
  '(suggestions self-evolution topic-specific git-history
                ...
                graph-questions graph-community)
...
Returns a compact lambda-notation string ready for the LLM."
  (let* ((target (or (cdr (assoc 'target vars)) "unknown"))
         ...
         (focus (cdr (assoc 'focus-line vars)))
          ...
          (graph-questions (cdr (assoc 'graph-questions vars)))
          (graph-community (cdr (assoc 'graph-community vars)))
...
exit codes\n")
      ...
      (if (or topic prev) (concat "PAST: " (or topic "")
       ...
       (if system-health (concat system-health "\n") "")
        ...
        (if graph-questions (concat graph-questions "\n") "")
        (if graph-community (concat graph-community "\n") "")
...
explore a different part of the file.\n"
              ...
              (validation-pipeline . ,(gptel-auto-workflow--load-skill-content "auto-workflow/validation-pipeline"))
                (research-findings . ,(if (funcall section-included-p 'research-findings)
                                           ...
                                           ""))
                (graph-questions . ,(if (funcall section-included-p 'graph-questions)
                                        (when (fboundp 'gptel-auto-workflow--graph-suggest-questions)
                                          (let ((qs (gptel-auto-workflow--graph-suggest-questions)))
                                            (if qs
                                                (concat "**Knowledge gaps to investigate**:\n"
                                                        (mapconcat (lambda (q) (concat "- " q)) qs "\n"))
                                              "")))
                                      ""))
                (graph-community . ,(when (fboundp 'gptel-auto-workflow--graph-community-for-target)
                                      (let ((comm-id (gptel-auto-workflow--graph-community-for-target target))
                                            (cohesion (when (fboundp 'gptel-auto-workflow--graph-cohesion-for-target)
                                                        (gptel-auto-workflow--graph-cohesion-for-target target))))
                                        (when comm-id
                                          (format "**Community context**: ID=%d, cohesion=%.2f"
                                                  comm-id (or cohesion 0.0))))))
