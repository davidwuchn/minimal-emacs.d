...
(defun gptel-tools-programmatic-register ()
  ...
  (when (fboundp 'gptel-make-tool)
    (gptel-make-tool
     :name "Programmatic"
:description
      "Execute restricted Emacs Lisp that orchestrates multiple existing tool calls. Use for tightly-coupled multi-step workflows; call tools as (tool-call \"ToolName\" :arg value ...); end the program with (result <expr>); not for arbitrary eval. Supports bounded loops: (dolist (item list) body...) for aggregations."
