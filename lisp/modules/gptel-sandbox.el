...
CALLBACK receives a plist with one of the keys `:continue' or `:result'."
  (unless (functionp callback)
    (error "Programmatic eval-statement requires a function callback, got: %S" callback))
  (unless (hash-table-p env)
...
CALLBACK receives final outcome plist."
  (unless (functionp callback)
    (error "Programmatic eval-progn requires a function callback, got: %S" callback))
  (unless (hash-table-p env)
