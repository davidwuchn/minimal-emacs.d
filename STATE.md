...
| `gptel-ext-auto-plan.el` | Auto-create planning file scaffolds for multi-step tasks | ~114 |
...
- **Split gptel-ext-context.el** (⚒): Split 428-line monolith into 3 focused modules. `gptel-ext-context-cache.el` (~296 lines): context-window caching — model-id helpers, normalize, cache load/put/seed, OpenRouter async fetch, auto-refresh timer. `gptel-ext-auto-plan.el` (~114 lines): auto-planning — step counting, planning-signal detection, safe directory resolution, scaffold file creation. `gptel-ext-context.el` (~108 lines): slimmed to auto-compact only — threshold/interval checks, LLM-based summarization via `compact` directive. All 3 byte-compile with zero warnings.
