...
### Recent Changes (v0.6.58)

- **ai-code-menu transient integration** (⚒): All ECA workspace/context/session commands now accessible via main `M-x ai-code-menu`. Dynamic status display (session ID, folder count). No need to remember `C-c e` prefix. Menu groups: ECA Workspace, ECA Context, ECA Shared Context, ECA Sessions.

### Recent Changes (v0.6.57)

- **Complete multi-project workspace support** (⚒): Fixed all 5 identified gaps. Gap 1: Added `eca-auto-switch-session` for automatic session switching based on project. Gap 3: Added cross-session context sharing (`eca-share-file-context`, `eca-share-repo-map-context`). Gap 4: Added `eca-session-dashboard` for visual session management. Gap 5: Added `ai-code-eca-auto-switch-backend` to auto-set backend on session switch. 60 tests passing.
...
- **Keep first ai-code Opencode vterm in Emacs state** (⊘): Hardened `lisp/init-evil.el` against the Evil/vterm startup race seen after restarting Emacs. `evil-collection-vterm` can reassert `insert` during the first terminal initialization even when `vterm-mode` is configured for Emacs state, so the config now schedules a next-tick `evil-emacs-state` correction from both `vterm-mode-hook` and `evil-local-mode-hook`, while also reapplying the `vterm-mode -> emacs` initial-state override after `evil-collection-vterm` loads.
- **Load gptel on demand for ai-code helpers** (⊘): Added an `ai-code-call-gptel-sync` advice in `lisp/init-ai.el` so deferred `gptel` setups still work when ai-code asks for helper generations like `:prompt` shell-command expansion, prompt classification, note headlines, or task filenames before any explicit `gptel` command has been run.
- **Preload ai-code's prompt bridge for `:` shell commands** (⊘): `ai-code-shell-cmd` can reach `ai-code-call-gptel-sync` from `ai-code-file.el` before `ai-code-prompt-mode.el` has been loaded, which makes `:list files` fall through as a literal shell command instead of invoking gptel. `lisp/init-ai.el` now loads `ai-code-prompt-mode` after `ai-code-file` so `:`-prefixed shell prompts reliably route through gptel command generation.
- **Teach `C-c a !` file runners to honor `:` prompts** (⊘): In file-visiting buffers, `ai-code-run-current-file-or-shell-cmd` still reaches `ai-code-run-current-file`, whose comint runner tried to execute `:list files` literally. `lisp/init-ai.el` now intercepts `ai-code--run-command-in-comint` and resolves colon-prefixed commands through `ai-code--generate-shell-command` first, so the same gptel shell-command flow works from `C-c a !` in ordinary file buffers too.
- **Use a fast helper model for ai-code sync gptel calls** (⊘): ai-code's `ai-code-call-gptel-sync` hard-times out after 30 seconds, which is a bad fit for the repo's default Moonshot reasoning model. `lisp/init-ai.el` now runs ai-code helper calls against DashScope `qwen3-coder-next` so shell-command generation, prompt classification, and other synchronous helper requests avoid slow reasoning stalls, and it now fails with a direct "AI shell command generation returned no command" error instead of falling through to `Command cannot be empty`.
- **Override ai-code's shell generator to sanitize gptel output** (⊘): Upstream `ai-code--generate-shell-command` asks gptel for a command, but if the model echoes the original request (for example `:list files`) the helper happily accepts that and tries to run it. `lisp/init-ai.el` now overrides the shell generator to demand a single command-only answer, strip fences, reject colon-prefixed echoes, preserve Dired placeholder expansion, and surface a direct `AI shell command generation failed` error when generation still does not yield a runnable command.
- **Pin Dirvish async subprocesses to the current Emacs binary** (⊘): Dirvish spawns `-Q -batch` subprocesses via `dirvish-emacs-bin` to collect directory metadata. On this machine it was resolving to an older app-bundle executable that occasionally returned truncated output, producing `Fetch dir data failed with error: (end-of-file)` when opening Dired. `lisp/init-tools.el` now resets `dirvish-emacs-bin` to `(concat invocation-directory invocation-name)` after Dirvish loads, so the async helper uses the exact running Emacs binary.
- **Enable ai-code's existing gptel integration** (⚒): `lisp/init-ai.el` now actually turns on ai-code's built-in gptel-assisted helpers instead of merely installing both packages side-by-side. Headline generation, prompt classification for auto-test routing, note-title generation, task filename generation, and `:`-prefixed shell-command generation now all use the existing `ai-code-call-gptel-sync` bridge, while ai-code itself stays on the `opencode` backend for primary execution. `ai-code-auto-test-type` is set to `ask-me` so gptel-based prompt classification now actively decides when the auto-test workflow should engage.
