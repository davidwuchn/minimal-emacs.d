...
- **Add Programmatic benchmark harness** (⊘): Added `lisp/modules/gptel-programmatic-benchmark.el` and `scripts/benchmark-programmatic.sh` to compare ordinary multi-tool chaining against a single Programmatic orchestration for a representative `Grep -> Read -> Read -> summarize` workflow. Current benchmark shows ~2.98x simulated end-to-end speedup and ~18.12% transcript-byte reduction by collapsing three tool turns into one.
