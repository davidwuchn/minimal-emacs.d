...
## Recent Updates
- **TREESIT-AUTO RECIPES**: Created `lisp/init-treesit.el` with custom `treesit-auto` recipes for Python, Rust, Clojure, and Elisp. Each recipe includes `:abi14-revision` tags to ensure compatibility with Emacs 30's ABI limitations, preventing the "version-mismatch: 15" errors. **VERIFIED**: Recipes successfully register with correct ABI14 revisions (Python v0.21.0, Rust v0.23.1, Elisp v1.2.0).
