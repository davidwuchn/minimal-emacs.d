;;; strategy-outcome-reasoning.el --- Outcome reasoning injection in prompt architecture -*- lexical-binding: t; -*-
;; Hypothesis: Injecting explicit reasoning between success/failure patterns improves learning
...
(defun strategy-outcome-reasoning--extract-failure-patterns (target)
  "Extract failure patterns from TARGET."
...
(defun strategy-outcome-reasoning--extract-axis-guidance (target)
  "Extract axis-specific guidance for TARGET."
...
(defun strategy-outcome-reasoning-build-prompt (target experiment-id max-experiments analysis baseline previous-results)
  "Build prompt using evolved-0004 reasoning injection architecture."
   (let* ((base-prompt (gptel-auto-experiment-build-prompt target experiment-id max-experiments analysis baseline previous-results))
          ...
          (failure-patterns (strategy-outcome-reasoning--extract-failure-patterns target))
          (evolved-0004-analysis (strategy-outcome-reasoning--generate-evolved-0004-analysis success-patterns failure-patterns))
          (axis-guidance (strategy-outcome-reasoning--extract-axis-guidance target)))
...
(defun strategy-outcome-reasoning-get-metadata ()
  "Return metadata for this strategy."
   (list :name "outcome-reasoning"
        :version "1.0"
        :hypothesis "Explicit reasoning between success and failure patterns improves learning signal"
        :axis "A"
        :components ["outcome-analysis" "success-extraction" "failure-context" "reasoning-injection"]))
