  ...
  (let ((examples nil))
    (dolist (result previous-results)
      (when (and (listp result)
                 (> (plist-get result :code-quality) 0.3)
                 (equal target (plist-get result :target)))
        (push (list :target (plist-get result :target)
                    :improvement (plist-get result :hypothesis)
                    :quality-gain (plist-get result :code-quality)
                    :pattern (plist-get result :exploration-axis))
              examples)))
    (nreverse examples)))
