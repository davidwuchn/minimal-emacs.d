...
(ert-deftest test-regen/execute-returns-no-success-when-experiment-unavailable ()
  ...
  (let ((original-override gptel-auto-workflow--experiment-prompt-override))
    (unwind-protect
        (progn
      ...
      (setq gptel-auto-workflow--experiment-prompt-override original-override))))

(ert-deftest test-regen/execute-success-path ()
  "--execute should return :success t when experiment is kept."
  (let ((mementum-calls nil)
        (original-override gptel-auto-workflow--experiment-prompt-override))
    (unwind-protect
        (progn
