...
(ert-deftest tdd/memory-schema/unified-graph/god-nodes-ranks-by-degree ()
  "`gptel-auto-workflow--unified-graph-god-nodes' returns the most
connected nodes in the unified graph by degree centrality.
Excludes file-level hub nodes (high-degree :file nodes) and isolates.
Returns alist of (node-key . degree) sorted by degree descending."
  (with-schema-test-env
   (cl-letf (((symbol-function 'gptel-auto-workflow--worktree-base-root)
              (lambda () gptel-auto-workflow--test-tmpdir))
             ((symbol-function 'gptel-auto-workflow--build-digital-twin)
     ...
     (let ((gptel-auto-workflow--unified-graph (make-hash-table :test 'equal))
       ...
       ;; Build a hub-A: 3 connections; a loner: 0
       (puthash (cons :entity "hub-A")
                `((:schema-neighbor ,(cons :entity "B") 0.9)
                  (:schema-neighbor ,(cons :entity "C") 0.9)
                  (:schema-neighbor ,(cons :entity "D") 0.9))
                gptel-auto-workflow--unified-graph)
       (puthash (cons :entity "B")
                `((:schema-neighbor ,(cons :entity "hub-A") 0.9))
                gptel-auto-workflow--unified-graph)
       (puthash (cons :entity "C")
                `((:schema-neighbor ,(cons :entity "hub-A") 0.9))
                gptel-auto-workflow--unified-graph)
       (puthash (cons :entity "D")
                `((:schema-neighbor ,(cons :entity "hub-A") 0.9))
                gptel-auto-workflow--unified-graph)
       (puthash (cons :entity "loner")
                nil
                gptel-auto-workflow--unified-graph)
       (let* ((result (gptel-auto-workflow--unified-graph-god-nodes 10))
              (hub-A-assoc (assoc (cons :entity "hub-A") result))
              (loner-assoc (assoc (cons :entity "loner") result)))
         (should hub-A-assoc)              ; hub-A is in top-N
         (should-not loner-assoc)           ; isolate excluded
         (should (>= (cdr hub-A-assoc) 3))  ; degree ≥ 3
         ;; Sorted by degree descending
         (let ((degrees (mapcar #'cdr result)))
           (should (equal degrees (sort (copy-sequence degrees) #'>)))))))))

(ert-deftest tdd/memory-schema/unified-graph/communities-group-connected ()
  "`gptel-auto-workflow--unified-graph-communities' detects communities
via label propagation. Two disconnected cliques should end up in
     ...
     (let ((gptel-auto-workflow--unified-graph (make-hash-table :test 'equal))
       ...
       (puthash (cons :entity "C")
                `((:neighbor ,(cons :entity "A") 1.0)
                  (:neighbor ,(cons :entity "B") 1.0))
                gptel-auto-workflow--unified-graph)
       ;; Clique 2: D-E-F (fully connected, isolated from clique 1)
       (puthash (cons :entity "D")
                `((:neighbor ,(cons :entity "E") 1.0)
                  (:neighbor ,(cons :entity "F") 1.0))
                gptel-auto-workflow--unified-graph)
       (puthash (cons :entity "E")
                `((:neighbor ,(cons :entity "D") 1.0)
                  (:neighbor ,(cons :entity "F") 1.0))
                gptel-auto-workflow--unified-graph)
       (puthash (cons :entity "F")
                `((:neighbor ,(cons :entity "D") 1.0)
                  (:neighbor ,(cons :entity "E") 1.0))
                gptel-auto-workflow--unified-graph)
       (let* ((communities (gptel-auto-workflow--unified-graph-communities 50))
              (comm-A (gethash (cons :entity "A") communities))
              (comm-B (gethash (cons :entity "B") communities))
              (comm-C (gethash (cons :entity "C") communities))
              (comm-D (gethash (cons :entity "D") communities))
              (comm-E (gethash (cons :entity "E") communities))
              (comm-F (gethash (cons :entity "F") communities)))
         (should comm-A) (should comm-B) (should comm-C)
         (should comm-D) (should comm-E) (should comm-F)
         ;; Within-clique: same community
         (should (= comm-A comm-B))
         (should (= comm-A comm-C))
         (should (= comm-B comm-C))
         (should (= comm-D comm-E))
         (should (= comm-D comm-F))
         ;; Across cliques: different communities
         (should (not (= comm-A comm-D))))))))

(ert-deftest tdd/memory-schema/unified-graph/surprising-connections-cross-community ()
  "`gptel-auto-workflow--unified-graph-surprising-connections' finds
cross-community edges with low weight (inferred or weak links).
Returns list of (source-id target-id edge-type weight community-diff)."
  (with-schema-test-env
   (cl-letf (((symbol-function 'gptel-auto-workflow--worktree-base-root)
              (lambda () gptel-auto-workflow--test-tmpdir))
             ((symbol-function 'gptel-auto-workflow--build-digital-twin)
     ...
     (let ((gptel-auto-workflow--unified-graph (make-hash-table :test 'equal))
           (gptel-auto-workflow--unified-graph-time (float-time)))
       ;; Clique 1: A-X-Y (strongly connected)
       (puthash (cons :entity "A")
                `((:strong ,(cons :entity "X") 1.0)
                  (:weak   ,(cons :entity "B") 0.1))
                gptel-auto-workflow--unified-graph)
       (puthash (cons :entity "X")
                `((:strong ,(cons :entity "A") 1.0)
                  (:strong ,(cons :entity "Y") 1.0))
                gptel-auto-workflow--unified-graph)
       (puthash (cons :entity "Y")
                `((:strong ,(cons :entity "X") 1.0))
                gptel-auto-workflow--unified-graph)
       ;; Clique 2: B-U-V (strongly connected, isolated from clique 1
       ;; except for the weak A-B edge)
       (puthash (cons :entity "B")
                `((:strong ,(cons :entity "U") 1.0)
                  (:weak   ,(cons :entity "A") 0.1))
                gptel-auto-workflow--unified-graph)
       (puthash (cons :entity "U")
                `((:strong ,(cons :entity "B") 1.0)
                  (:strong ,(cons :entity "V") 1.0))
                gptel-auto-workflow--unified-graph)
       (puthash (cons :entity "V")
                `((:strong ,(cons :entity "U") 1.0))
                gptel-auto-workflow--unified-graph)
       (let ((surprises (gptel-auto-workflow--unified-graph-surprising-connections 10)))
         (should surprises)
         ;; The weak cross-community A-B edge should appear
         (should (cl-some (lambda (s) (and (equal (nth 0 s) "A")
                                          (equal (nth 1 s) "B")))
                         surprises)))))))
