...
(ert-deftest regression/auto-workflow/staging-verification-failure-resets-staging ()
  "Failed staging verification should reset staging before the next target."
  (let ((reset-count 0)
        completion)
...
(ert-deftest regression/auto-workflow/staging-push-failure-resets-staging ()
  "Failed staging push should also reset staging before the next target."
  (let ((reset-count 0)
        completion)
    (cl-letf (((symbol-function 'gptel-auto-workflow--assert-main-untouched)
               (lambda (&rest _) t))
