...
(ert-deftest eca-bridge/extensions/which-session ()
  "Bridge should provide which-session command."
  (let ((source (or (ignore-errors
                      (with-temp-buffer
                        (insert-file-contents "lisp/ai-code-eca-bridge.el")
                        (buffer-string)))
                    "")))
    (should (string-match-p "defun ai-code-eca-which-session" source))))

(ert-deftest eca-bridge/extensions/mode-line-indicator ()
  "Bridge should provide mode-line indicator."
  (let ((source (or (ignore-errors
                      (with-temp-buffer
                        (insert-file-contents "lisp/ai-code-eca-bridge.el")
                        (buffer-string)))
