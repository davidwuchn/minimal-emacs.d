...
(ert-deftest test-tools-memory/resolve-path-adds-extension ()
...
(ert-deftest test-tools-memory/resolve-path-keeps-extension ()
  "Path resolution should keep existing .md extension."
  (let ((gptel-tools-memory--cached-root "/tmp/project"))
    (let ((path (gptel-tools-memory--resolve-path "my-insight.md")))
      (should (string-suffix-p ".md" path)))))

(ert-deftest test-tools-memory/resolve-path-knowledge ()
...
(ert-deftest test-tools-memory/resolve-path-memory ()
